...
@section('content')
  ...
  <div class="row">
    ...
    <div class="col-lg-3 col-6">
      ...
      <div class="small-box bg-gradient-primary">
        <div class="inner">
          <h3>
            {{ (\App\Models\Producto::where('tipo', 'insumo')->sum('existencia') <= 0) ? 0 : \App\Models\Producto::where('tipo', 'insumo')->sum('existencia')}}
          </h3>
  ...
  <div class="row">
    <div class="col-md-6 mb-4">
      <div class="card h-100 shadow-sm border-0">
        <div class="card-header bg-gradient-info text-white py-3">
          <h6 class="m-0 font-weight-bold"><i class="fas fa-chart-bar mr-1"></i> Solicitudes Resueltas por Mes</h6>
        </div>
        <div class="card-body">
    ...
    $q->where('estatus', 'Finalizada')->orWhere('estatus', 'Finalizado'); })->count()}}" data-febrero="{{\App\Models\Factura::where(DB::raw("MONTH(created_at)"), 2)->where(function ($q) {
    $q->where('estatus', 'Finalizada')->orWhere('estatus', 'Finalizado'); })->count()}}" data-marzo="{{\App\Models\Factura::where(DB::raw("MONTH(created_at)"), 3)->where(function ($q) {
    $q->where('estatus', 'Finalizada')->orWhere('estatus', 'Finalizado'); })->count()}}" data-abril="{{\App\Models\Factura::where(DB::raw("MONTH(created_at)"), 4)->where(function ($q) {
    $q->where('estatus', 'Finalizada')->orWhere('estatus', 'Finalizado'); })->count()}}" data-mayo="{{\App\Models\Factura::where(DB::raw("MONTH(created_at)"), 5)->where(function ($q) {
    $q->where('estatus', 'Finalizada')->orWhere('estatus', 'Finalizado'); })->count()}}" data-junio="{{\App\Models\Factura::where(DB::raw("MONTH(created_at)"), 6)->where(function ($q) {
    $q->where('estatus', 'Finalizada')->orWhere('estatus', 'Finalizado'); })->count()}}" data-julio="{{\App\Models\Factura::where(DB::raw("MONTH(created_at)"), 7)->where(function ($q) {
    $q->where('estatus', 'Finalizada')->orWhere('estatus', 'Finalizado'); })->count()}}" data-agosto="{{\App\Models\Factura::where(DB::raw("MONTH(created_at)"), 8)->where(function ($q) {
    $q->where('estatus', 'Finalizada')->orWhere('estatus', 'Finalizado'); })->count()}}" data-septiembre="{{\App\Models\Factura::where(DB::raw("MONTH(created_at)"), 9)->where(function ($q) {
    $q->where('estatus', 'Finalizada')->orWhere('estatus', 'Finalizado'); })->count()}}" data-octubre="{{\App\Models\Factura::where(DB::raw("MONTH(created_at)"), 10)->where(function ($q) {
    $q->where('estatus', 'Finalizada')->orWhere('estatus', 'Finalizado'); })->count()}}" data-noviembre="{{\App\Models\Factura::where(DB::raw("MONTH(created_at)"), 11)->where(function ($q) {
    $q->where('estatus', 'Finalizada')->orWhere('estatus', 'Finalizado'); })->count()}}" data-diciembre="{{\App\Models\Factura::where(DB::raw("MONTH(created_at)"), 12)->where(function ($q) {
    $q->where('estatus', 'Finalizada')->orWhere('estatus', 'Finalizado'); })->count()}}">
            </canvas>
          </div>
        </div>
      </div>
